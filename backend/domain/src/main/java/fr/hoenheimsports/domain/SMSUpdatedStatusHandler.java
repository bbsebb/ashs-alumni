...
public class SMSUpdatedStatusHandler implements HandleSMSUpdatedStatus {
     ...
     * </ol>
     * 
     * @param command la commande contenant les informations de mise à jour du statut SMS
     ...
     */
    @Override
    public void execute(SMSUpdatedStatusCommand command) {
        var smsHistory = retrieveSMSHistory(command);

        if (smsHistory.status() == command.smsStatusUpdate().smsStatus()) {
            return;
        }

        var formerTeammate = retrieveFormerTeammate(command);
        var updatedSmsHistory = updateSMSHistoryWithStatus(smsHistory, command);
     ...
     * 
     * @param command la commande contenant l'identifiant externe du SMS
     ...
     */
    private SMSHistory retrieveSMSHistory(SMSUpdatedStatusCommand command) {
        return smsHistoryRepository.findByExternalID(command.smsStatusUpdate().externalSmsId())
                .orElseThrow(() -> new SMSHistoryRepositoryException(
                        "L'historique des sms n'a pas été trouvé avec l'id : %s".formatted(command.smsStatusUpdate().externalSmsId())));
    }

    /**
     * Récupère l'ancien coéquipier à partir de son identifiant.
     * 
     * @param command la commande contenant l'identifiant de l'ancien coéquipier
     ...
     */
    private FormerTeammate retrieveFormerTeammate(SMSUpdatedStatusCommand command) {
        return formerTeammateRepository.findById(UUID.fromString(command.formerTeammateReference().formerTeammateId()))
                .orElseThrow(() -> new FormerTeammateRepositoryException(
                        "Le contact n'a pas été trouvé avec l'id %s".formatted(command.formerTeammateReference().formerTeammateId())));
    }
     ...
     * @param command la commande contenant le nouveau statut et les informations d'erreur
     * @return l'historique SMS mis à jour avec le nouveau statut et éventuellement un message d'erreur
     */
    private SMSHistory updateSMSHistoryWithStatus(SMSHistory originalSmsHistory, SMSUpdatedStatusCommand command) {
        var updatedSmsHistory = originalSmsHistory.withStatus(command.smsStatusUpdate().smsStatus());

        if (updatedSmsHistory.hasFailed()) {
            updatedSmsHistory = updatedSmsHistory.withErrorMessage(
                    "Erreur de type %s : %s".formatted(command.smsStatusUpdate().errorCode(), command.smsStatusUpdate().errorMessage()));
        }
